...
async def edit_model(message: types.Message, state: FSMContext):
    ...
    logger.info_from_handlers(Loggers.INCOMING.value, tg_user_id, name_func, message_from_user, "изменил бренд машины")
    await message.answer(
        text="Введите модель:",
        reply_markup=buttons.car_edit_cancel,
    )
...
async def delete_result_handling(message: types.Message, state: FSMContext):
    ...
    if message.text == "Да":
        ...
        logger.info_from_handlers(Loggers.INCOMING.value, tg_user_id, name_func, message_from_user, "автомобиль удален")
    # pylint: disable=R0801
        ...
        logger.warning(Loggers.APP.value, tg_user_id, name_func, message_from_user, "возникла ошибка при удалении авто")
